...
# This file was *autogenerated* from the file /Users/wojtek/Library/CloudStorage/GoogleDrive-w.politarczyk@uw.edu.pl/My Drive/mat/git-projects/signature_function/gaknot_lib/LT_signature.sage
from sage.all_cmdline import *   # import sage library

_sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_2 = Integer(2)#!/usr/bin/env sage -python
...
if __name__ == '__main__':
    ...
    from .utility import import_sage
    package = __name__.rsplit('.', _sage_const_1 )[_sage_const_0 ]
...
def LT_signature_torus_knot(p, q):
    ...
    if p <= _sage_const_1  or q <= _sage_const_1 :
        raise ValueError(f'Parameters p and q must be >1. Got (p,q) = ({p}, {q}).')

    if math.gcd(p, q) != _sage_const_1 :
    ...
    roots = [Integer(i) / (p * q) for i in range(_sage_const_1 , p * q)]
    jumps = [x for x in roots if mod_one(p * x) != _sage_const_0  and mod_one(q * x) != _sage_const_0 ]
...
def LT_signature_iterated_torus_knot(desc):
    ...
    for i, el in enumerate(desc):
        ...
        if not isinstance(el, (list, tuple)) or len(el) != _sage_const_2 :
...
def LT_signature_generalized_algebraic_knot(desc):
    ...
    for i, element in enumerate(desc):
        ...
        if not isinstance(element, (list, tuple)) or len(element) != _sage_const_2 :
        ...
        if sign != _sage_const_1  and sign != -_sage_const_1 :
