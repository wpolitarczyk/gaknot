...
# This file was *autogenerated* from the file /Users/wojtek/Library/CloudStorage/GoogleDrive-w.politarczyk@uw.edu.pl/My Drive/mat/git-projects/signature_function/gaknot_lib/H1_branched_cover.sage
...
class BranchedCoverHomology:
    ...
    def _from_torus_knot(p, q, N):
        ...
        for i in range(d):
            C[i, d - _sage_const_1 ] = -coeffs[i]
            
        N_mod = N % (p * q)
        I = matrix.identity(ZZ, d)
        M = (C**N_mod) - I
        
        D = M.smith_form()[_sage_const_0 ]
    ...
    def _from_iterated_torus_knot(cls, cable_sequence, N):
        ...
        for i in range(len(cable_sequence) - _sage_const_1 , -_sage_const_1 , -_sage_const_1 ):
            ...
            cleaned_factors = sorted([f for f in base_factors if f != _sage_const_1 ])
    ...
    def __getitem__(self, i):
        ...
        if int(i) < _sage_const_0  or int(i) >= len(self._decomposition):
    ...
    def betti_number(self):
        ...
        return self.invariant_factors.count(_sage_const_0 )
    ...
    def __str__(self):
        ...
        for comp in self._decomposition:
            ...
            group_str = " \u2295 ".join([f"Z/{f}Z" if f != _sage_const_0  else "Z" for f in factors])
            
            # Create a label for the knot part (e.g. "T(2,3)")
            desc_str = "T(" + "; ".join([f"{p},{q}" for p, q in comp['description']]) + ")"
            sign_str = "-" if comp['sign'] == -_sage_const_1  else ""
